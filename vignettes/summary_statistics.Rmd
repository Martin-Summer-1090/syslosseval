...
data_2016 <- eba_exposures_2016 %>% select(LEI_code, Bank_name, Exposure, Bond_Amount, Total_Amount)
data_2020 <- eba_exposures_2020 %>% select(LEI_code, Bank_name, Exposure, Bond_Amount, Total_Amount) 
```
...
bonds_2016 <- data_2016 %>% 
  ...
  group_by(LEI_code) %>% 
  summarise(Bonds = sum(Bond_Amount)) %>%
  mutate(Bonds = Bonds/1000) %>% 
  select(LEI_code, Bonds) %>% 
  ungroup()
  
ta_2020 <- data_2020 %>% 
  filter(Exposure == "Total assets", Total_Amount >= 30000) %>% 
  select(LEI_code, Total_Amount) %>% 
  rename(Total_assets = Total_Amount) %>% 
...
bonds_2020 <- data_2020 %>% 
  ...
  group_by(LEI_code) %>% 
  summarise(Bonds = sum(Bond_Amount)) %>%
  mutate(Bonds=Bonds/1000) %>%  # change units to billions
  select(LEI_code, Bonds) %>% 
  ungroup()
...
```{r}
summary_data_2020_without_outlier <- summary_data_2020[-88,]
```


```{r}
hist_leverage_2020 <- ggplot(data = summary_data_2020_without_outlier, mapping= aes(leverage)) + 
  geom_histogram(bins=15, color = "black", fill = "white", boundary = 0)  + 
  labs(x = "Leverage: Total assets (unweigthed) to core tier 1 equity", y = "Number of Banks") +
  geom_vline(xintercept = 33, color = "red", linetype = "dashed") 
