...
#'
#' @return a dataframe with the bon exposures for DE, ES, IT, FR, UK, US and total
#' @export
...
# Filter Data for Exposure and Country and select bond amount
  ...
  data %>% dplyr::filter(.data$Exposure == "Central banks and central governments",
                         .data$Country %in% c("Total", "DE", "ES", "FR", "IT", "JP", "UK", "US")) %>%
           dplyr::select(.data$LEI_code,
                         .data$Country_code,
                         .data$Bank_name,
                         .data$Period,
                         .data$Country,
